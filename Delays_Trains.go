...
loop:
	...
	fmt.Printf("%d packets transmitted, %d received, %.2f%% packet loss, time %d secs\n", sent, received, float32(sent-received)/float32(sent), int(elapsed/time.Second))
	...
	/*
		...
		for _, value_rrt := range p.RTTS {
			fmt.Fprintf(file, "%.2f\n", float32(value_rrt)/float32(time.Millisecond))
		}

		//File with Sequence Numbers
...
func main() {
	...
	flag.Float64Var(&rate, "r", 0.0, "rate parameter, number of events, in average, present in a minute")
